...
const StockChart: React.FC = () => {
  ...
  const fetchHistoricalData = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`https://apple-backend1.onrender.com/history?days=${daysToShow}`);
  ...
  const fetchPredictions = useCallback(async (days: number = 7) => {
    try {
      const response = await axios.post('https://apple-backend1.onrender.com/predict_multi', { days });
